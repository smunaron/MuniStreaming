...
export default function ShowCard({ show, size = "md" }: ShowCardProps) {
  ...
  return (
    <Link href={`/shows/${show.id}`} className="group block shrink-0">
      <div className={`relative ${dims.cls} rounded-xl overflow-hidden bg-[#160429]`}>
        <Image
          src={show.thumbnail}
          alt={show.title}
          fill
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="bg-pink-600/80 backdrop-blur-sm rounded-full p-3 border border-pink-400/50">
            <Play className="text-white fill-white" size={24} />
          </div>
        </div>
        {/* Category badge */}
        <div className="absolute top-2 left-2">
          <span className="text-xs font-bold px-2 py-0.5 bg-pink-600/90 text-white rounded-full uppercase tracking-wide">
            {show.category === "movie" ? "Film" : "Serie"}
          </span>
        </div>
